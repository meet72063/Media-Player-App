  ...
  return (
    <div className='sm:ml-10 '>
      <div className='  flex gap-2'>
        ...
        {/* <button className='sm:pl-5 sm:pr-5 p-2 bg-red-500 text-black rounded-lg   text-lg'>search</button> */}
      </div>
      {searchInput && <div className=' w-full mt-1   '>
        <div className='bg-white  relative w-full  rounded-md '>

          <div className='flex justify-end pr-5 items-center p-3 cursor-pointer' onClick={closeBtnHandler}><Close /></div>
  ...
  return (
    <>
      <div className='inline-block text-black  rounded-md'>
        <span className='cursor-pointer' onClick={handlePlay}><FontAwesomeIcon icon={faMusic}  /> {track.name} ({track.artist})</span>
      </div>
    </>
  )
}